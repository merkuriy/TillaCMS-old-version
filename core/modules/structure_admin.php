...
class modules_structure_admin{
	...
	//==================================================================================================
	// Функция построения ниспадающего меню
	function buildDDMenu($author = ''){
		$sql = sys::sql("SELECT `id`,`title` FROM `prefix_Sections` WHERE `parent_id`='0'",0);			// Выбираем корни из БД
		while ($res = mysql_fetch_array($sql)){
			$SEND['option'] .= admin::draw('structure/ddMenu.element',$res);							// выводим элементы ниспадающего списка
		}
		$out .= admin::draw('structure/ddMenu',$SEND);													// выводим ниспадающий список
...
/*
	...
	function buildTree($id='1',$author=''){

		$sql = sys::sql("SELECT `title` FROM `prefix_Sections` WHERE `id`='$id'",0);					// Выбираем название отображаемого корня
		$title = mysql_result($sql,0);

		if ($author=='admin'){
			$nodes[] = array('label'=>$title, 'id'=>$id, 'items'=>'ajax', 'url'=>'?module=structure&action=findChild&id='.$id.'&author=admin');
			echo json_encode($nodes);
		}else{
			return admin::draw('structure/tree',$SEND);
		}
